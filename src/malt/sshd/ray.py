...
def ray_triangle_intersect(triangle, origin, direction):
    ...
    if det > 0:
        ...
        T = triangle[0] - origin
        det = -det
    # if the determine is near zero, the ray is parallel with the plane of the
    # triangle
    if det < 1e-04:
        return False, -1
...
def ray_trimesh_intersect_first(vertices, facet, origin, direction):
    ...
    ..note: here first means t value is smallest(i.e. the intersection is
            closest to the origin), so the given order of the facet doesn't
            matter
...
def ray_trimesh_intersect_last(vertices, facet, origin, direction):
    ...
    for k in range(len(facet)):
        triangle = vertices[facet[k]]
        judge[k], tvalue[k] = ray_triangle_intersect(triangle, origin,
                                                     direction)
    if tvalue[judge].shape == (0,):
        return False, -1, -1
...
def ray_trimesh_intersect_any(vertices, facet, origin, direction):
    ...
    for k in range(len(facet)):
        triangle = vertices[facet[k]]
        if ray_triangle_intersect(triangle, origin, direction)[0] is True:
            return True
    return False
